...
class LabelDeleteView(LoginRequiredMixin,
    ...
    def test_func(self):
        return self._is_have_task_with_this_label()

    def _is_have_task_with_this_label(self):
        return not Task.objects.filter(labels__pk=self.kwargs['pk']).exists()

    def handle_no_permission(self):
        messages.error(self.request, _DELETE_LABEL_ERROR_MESSAGE)
        return redirect(reverse_lazy('labels:list'))
